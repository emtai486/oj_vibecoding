...
### WEB-014 登录用户提交错误答案返回 Wrong Answer
...
| 预期结果 | `#submit-result` 最终显示 `Wrong Answer`；不会写入新的通过状态。 |

### WEB-015 登录用户提交编译错误返回 Compile Error
...
| 预期结果 | 页面展示 `Compile Error`。 |

### WEB-016 空代码提交返回 Compile Error
...
| 预期结果 | 页面展示 `Compile Error`；不进入通过状态。 |
...
| 预期结果 | 即使数值正确，因为输出缺少期望换行，结果仍为 `Wrong Answer`。 |
...
### WEB-019 超时代码返回 Time Limit Exceeded
...
| 预期结果 | 页面最终显示 `Time Limit Exceeded`；服务没有卡死；之后仍可继续请求 `/health` 或打开题目列表。 |

### WEB-020 内存超限代码返回 Memory Limit Exceeded
...
| 预期结果 | 页面最终显示 `Memory Limit Exceeded`；服务仍可继续响应。 |

### WEB-021 输出超限代码返回 Output Limit Exceeded
...
| 预期结果 | 页面最终显示 `Output Limit Exceeded`；服务没有返回超大页面内容。 |
