...
4. 每条自动化用例建议使用独立浏览器上下文，避免 Cookie 和 `localStorage` 相互污染。

5. 如测试管理员新增/删除题目，题目标题应带唯一后缀，例如：

```text
Web Auto Problem ${timestamp}
```
...
### 5.5 float_1 通过代码：Average Score
...
| --- | --- |
| 优先级 | P1 |
| 前置条件 | 使用唯一用户名，例如 `web_user_${timestamp}` |
| 操作步骤 | 1. 打开 `/register.html`。<br>2. 输入唯一用户名和密码 `password`。<br>3. 提交表单。<br>4. 等待跳转到 `/login.html`。<br>5. 使用新账号登录。 |
...
| 优先级 | P1 |
| 前置条件 | 已存在用户 `user1` |
| 操作步骤 | 1. 打开 `/register.html`。<br>2. 输入用户名 `user1`、密码 `password`。<br>3. 提交表单。 |
| 预期结果 | 页面显示 `username exists`；不跳转到登录成功状态。 |
...
| 优先级 | P0 |
| 前置条件 | 已执行 WEB-011 |
| 操作步骤 | 1. 在浏览器上下文中读取 `localStorage.getItem('oj_problem_status')`。<br>2. 返回 `/problems.html`。<br>3. 查找题目 1 的完成状态。 |
...
| --- | --- |
| 优先级 | P1 |
| 前置条件 | 已登录普通用户且题目 1 已通过 |
| 操作步骤 | 1. 点击导航中的“退出”。<br>2. 等待页面刷新或回到未登录状态。<br>3. 打开 `/problems.html`。 |
...
| 优先级 | P0 |
| 前置条件 | 已登录管理员 |
| 操作步骤 | 1. 打开 `/admin/new-problem.html`。<br>2. 填写唯一标题 `Web Auto Problem ${timestamp}`。<br>3. 选择难度 `easy`。<br>4. 选择比较模式 `strict`。<br>5. 填写题目描述、输入格式、输出格式。<br>6. 填写样例输入 `2 3\n`、样例输出 `5\n`。<br>7. 时间限制填写 `1000`，内存限制填写 `131072`。<br>8. 隐藏测试用例 JSON 填写 `[{"input":"7 8\n","expected_output":"15\n"},{"input":"-2 5\n","expected_output":"3\n"}]`。<br>9. 点击“保存题目”。 |
| 预期结果 | 表单提交成功；跳转到 `/admin/index.html`；管理员题目列表出现新题目。 |
...
| 优先级 | P1 |
| 前置条件 | 已登录管理员 |
| 操作步骤 | 1. 打开 `/admin/new-problem.html`。<br>2. 填写其他必填字段。<br>3. 将隐藏测试用例 JSON 填写为 `[]`。<br>4. 提交表单。 |
...
| 优先级 | P1 |
| 前置条件 | 已登录管理员 |
| 操作步骤 | 1. 打开 `/admin/new-problem.html`。<br>2. 填写其他必填字段。<br>3. 将隐藏测试用例 JSON 填写为 `{"broken":`。<br>4. 提交表单。 |
...
| --- | --- |
| 优先级 | P0 |
| 前置条件 | 已执行 WEB-026，记录新题目标题 |
| 操作步骤 | 1. 打开 `/problems.html`。<br>2. 查找新题目标题。<br>3. 点击进入详情页。<br>4. 检查题面展示样例 `2 3` 和 `5`。<br>5. 检查页面不包含隐藏测试输入 `7 8`。 |
| 预期结果 | 新题目前台可见；详情页只展示样例，不暴露隐藏测试用例。 |
...
| --- | --- |
| 优先级 | P0 |
| 前置条件 | 已执行 WEB-026；已登录普通用户或管理员 |
| 操作步骤 | 1. 打开新增题目的详情页。<br>2. 输入只输出样例答案 `5` 的代码。<br>3. 点击提交。 |
| 预期结果 | 判题失败，证明服务使用隐藏测试用例而不是只使用样例。 |
...
| --- | --- |
| 优先级 | P0 |
| 前置条件 | 已执行 WEB-026，新增题目仍存在；已登录管理员 |
| 操作步骤 | 1. 打开 `/admin/index.html`。<br>2. 找到新增题目所在行。<br>3. 点击该行的“删除”按钮。<br>4. 等待列表刷新。 |
...
| --- | --- |
| 优先级 | P0 |
| 前置条件 | 已执行 WEB-031，记录被删除题目 ID |
| 操作步骤 | 1. 打开 `/problem.html?id=${deletedProblemId}`。<br>2. 等待详情请求完成。 |
| 预期结果 | 页面不展示原题目内容；接口返回 404；页面显示加载失败或等价错误状态。 |
...
1. 每个普通用户注册用例使用唯一用户名，避免重复数据冲突。
2. 管理员新增题目用例必须记录新题目 ID 或唯一标题。
3. 管理员新增题目的测试结束后必须删除该题目。
4. 每个浏览器上下文开始前清理：
...
5. 如果测试中断导致残留题目，可重新运行数据库初始化脚本恢复基础数据：
