...
- 系统可在当前项目服务器 Ubuntu 22.04 LTS + g++ + MySQL 8 环境运行
...
- [x] 编写启动脚本
- [x] 编写数据库初始化脚本
- [x] 在当前项目服务器 Ubuntu 22.04.5 LTS 验证（已执行 `scripts/deploy_verify.sh --strict-os config/app.conf` / `make deploy-verify-strict`）
- [x] 验证 g++ 编译运行
- [x] 验证 MySQL 连接
- [x] 验证前端静态资源本地加载
- [x] 验证普通用户登录后才能提交代码
...
- [x] 用户看不到隐藏测试用例
- [x] 未登录用户不能提交代码
- [x] 未登录用户调用提交接口时返回未登录或无权限
- [x] 普通用户可以登录
- [x] 登录普通用户可以编辑 C++ 代码
- [x] 登录普通用户可以提交代码
- [x] 正确代码返回 `accepted`
- [x] 错误代码返回 `wrong_answer`
- [x] 编译错误返回 `compile_error`
- [x] 超时代码返回 `time_limit_exceeded`
- [x] 登录普通用户通过后的题目状态保存到 localStorage

### 13.3 管理员验收

- [x] 管理员可以登录
- [x] 管理员登录状态通过 session/cookie 保持
- [x] 管理员可以新增题目
- [x] 新增题目可以包含样例测试用例
- [x] 新增题目可以包含隐藏测试用例
- [x] 新增题目后普通用户可以看到
...
- [x] 系统可以捕获 stdout
- [x] strict 模式可以严格比较输出
- [x] float_1 模式可以按小数点后一位比较
- [x] 超过 1 秒默认时间限制时返回 `time_limit_exceeded`
- [x] 超过 128 MB 默认内存限制时返回 `memory_limit_exceeded`
- [x] 多人同时提交时最多运行 2-4 个判题进程
